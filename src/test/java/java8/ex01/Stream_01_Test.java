...
public class Stream_01_Test {
    ...
    public void test_stream_filter_and_match() throws Exception {
        ...
        List<Order> result = orders.stream().filter(order -> order.getCustomer().getFirstname() == "Johnny" && order.getPizzas().stream().anyMatch(p -> p.getPrice() >= 1300)).collect(Collectors.toList());
    ...
    public void test_stream_min() throws Exception {
        ...
        Optional<Pizza> result = pizzas.stream().filter(p -> p.getPrice() >= 950).min((p1, p2) -> p1.getPrice().compareTo(p2.getPrice()));
    ...
    public void test_sorted() throws Exception {
        ...
        List<Order> result = orders.stream().sorted((ord1, ord2) -> ord1.getCustomer().getLastname().compareTo(ord2.getCustomer().getLastname())).collect(Collectors.toList());
